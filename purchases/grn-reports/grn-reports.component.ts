...
export class GrnReportsComponent implements OnInit {
  ...
  GRNReport:FormGroup;
  IsRequestFromDashboard:Boolean=false;
  DBResult:any=[];
  minDate='';
  lstGRN=[];
  ...
  constructor(private router:Router,private formBuilder: FormBuilder,private datePipe:DatePipe,private APICall:APICallingService,private store: Store<any>) {
    ...
    this.GRNReport =this.formBuilder.group({
...
$('.js-select2').on("select2:unselecting", function(e){
 ...
 that.SelectedYear.splice(myIndex, 1)
  ...
  get f(){
    return this.GRNReport.controls;
  }
  ...
  if(this.FinancialYearxml==''){
   ...
   if(this.GRNReport.errors==null && this.f.FromDate.value <= this.f.ToDate.value && this.f.VendorId.value !='') {
    debugger;
    ...
    this.DBResult=[];
    this.lstGRN=[];
  this.APICall.DBCalling("GRNReports", xml1, "", "", "").subscribe(
    (res: Response) => {
debugger;
      $("#loaderParent").hide();
      ...
      if(this.DBResult =='<NewDataSet><Table1></Table1></NewDataSet>'){
        ...
        this.lstGRN=this.DBResult.Table;  
      }else{

      }
    });
 
  }else if(this.GRNReport.errors.notValid=='true'  || this.f.ToDate.value==this.FinancialEndYear ){
    this.submitted=true;
  }else{
