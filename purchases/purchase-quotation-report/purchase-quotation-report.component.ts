...
@Component({
  selector: 'app-purchase-quotation-report',
  templateUrl: './purchase-quotation-report.component.html',
  styleUrls: ['./purchase-quotation-report.component.css'],
  providers:[DatePipe]
})
export class PurchaseQuotationReportComponent implements OnInit {


  FinancialstartYear : string;
  PurchaseQuotationReport:FormGroup;
  IsRequestFromDashboard:Boolean=true;
  DBResult:any=[];
  minDate='';
  lstPurchaseQuotation=[];
FinancialYearxml:string=''
  ...
  constructor(private router:Router,private formBuilder: FormBuilder,private datePipe:DatePipe,private APICall:APICallingService,private store: Store<any>) { 
    ...
    this.PurchaseQuotationReport =this.formBuilder.group({
...
$('.js-select2').on("select2:unselecting", function(e){
 ...
 that.SelectedYear.splice(myIndex, 1)
  ...
  get f(){
    return this.PurchaseQuotationReport.controls;
  }
  ...
  if(this.FinancialYearxml==''){
   ...
   if(this.PurchaseQuotationReport.errors==null && this.f.FromDate.value <= this.f.ToDate.value && this.f.VendorId.value !='') {
    debugger;
  ...
  this.APICall.DBCalling("PurchaseQuotaReport", xml1, "", "", "").subscribe(
    (res: Response) => {
debugger;
      $("#loaderParent").hide();
      ...
      if(this.DBResult =='<NewDataSet><Table1></Table1></NewDataSet>'){
        ...
        this.lstPurchaseQuotation=this.DBResult.Table;  
      }else{

      }
    });
 
  }else if(this.PurchaseQuotationReport.errors.notValid=='true'  || this.f.ToDate.value==this.FinancialEndYear ){
    this.submitted=true;
  }else{
