...
@Component({
  selector: 'app-purchaseinvoicereport',
  templateUrl: './purchaseinvoicereport.component.html',
  styleUrls: ['./purchaseinvoicereport.component.css'],
  providers:[DatePipe]
})
export class PurchaseinvoicereportComponent implements OnInit {
  ...
  PurchaseInvoiceReport:FormGroup;
  ...
  { 
    ...
    this.PurchaseInvoiceReport =this.formBuilder.group({
...
$('.js-select2').on("select2:unselecting", function(e){
 ...
 that.SelectedYear.splice(myIndex, 1)
  ...
  get f(){
    return this.PurchaseInvoiceReport.controls;
  }
  ...
  if(this.FinancialYearxml==''){
   ...
   if(this.PurchaseInvoiceReport.errors==null && this.f.FromDate.value <= this.f.ToDate.value && this.f.VendorId.value !='') {
    debugger;
  ...
  this.APICall.DBCalling("PurchaseInvoiceReport", xml1, "", "", "").subscribe(
    (res: Response) => {
debugger;
      $("#loaderParent").hide();
      ...
      if(this.DBResult =='<NewDataSet><Table1></Table1></NewDataSet>'){
        ...
        this.lstPurchaseInvoice=this.DBResult.Table;  
      }else{

      }
    });
 
  }else if(this.PurchaseInvoiceReport.errors.notValid=='true'  || this.f.ToDate.value==this.FinancialEndYear ){
    this.submitted=true;
  }else{
