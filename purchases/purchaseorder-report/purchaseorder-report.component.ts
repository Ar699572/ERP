...
export class PurchaseorderReportComponent implements OnInit {
  ...
  constructor(private router:Router,private formBuilder: FormBuilder,private datePipe:DatePipe,private APICall:APICallingService,private store: Store<any>)
   { 
    ...
    this.PurchaseorderReport =this.formBuilder.group({
...
$('.js-select2').on("select2:unselecting", function(e){
 ...
 that.SelectedYear.splice(myIndex, 1)
  ...
  if(this.FinancialYearxml==''){
   ...
   if(this.PurchaseorderReport.errors==null && this.f.FromDate.value <= this.f.ToDate.value && this.f.VendorId.value !='') {
    debugger;
  ...
  this.APICall.DBCalling("PurchaseorderReport", xml1, "", "", "").subscribe(
    (res: Response) => {
debugger;
      $("#loaderParent").hide();
      ...
      if(this.DBResult =='<NewDataSet><Table1></Table1></NewDataSet>'){
        ...
        this.lstPurchaseorder=this.DBResult.Table;  
      }else{

      }
    });
 
  }else if(this.PurchaseorderReport.errors.notValid=='true'  || this.f.ToDate.value==this.FinancialEndYear ){
    this.submitted=true;
  }else{
...
get f(){
  return this.PurchaseorderReport.controls;
}
}
