...
export class SalesReturnReportComponent implements OnInit {
  ...
  constructor(private router:Router,private formBuilder: FormBuilder,private datePipe:DatePipe,private APICall:APICallingService,private store: Store<any>) {
    ...
    this.SalesReturnReport =this.formBuilder.group({
...
this.LoadFinancialyear()
  ...
  get f(){
    return this.SalesReturnReport.controls;
  }
  LoadCustomer(){
    debugger;
...
$('.js-select2').on("select2:unselecting", function(e){
  debugger;
  console.log(e)
  var text = ((<any>e).params.args.data.id);
 var myIndex= that.SelectedYear.indexOf(text)
 that.SelectedYear.splice(myIndex, 1)
  ...
  GetCustomerReturns(){
   ...
   if(this.SalesReturnReport.errors==null && this.f.FromDate.value <= this.f.ToDate.value || this.f.CustomerId.value !='') {
    debugger;
    ...
    this.DBResult=[];
    this.lstSalesReturn=[];
  this.APICall.DBCalling("SalesReturnsReport", xml1, "", "", "").subscribe(
    (res: Response) => {
debugger;
      $("#loaderParent").hide();
      ...
      if(this.DBResult =='<NewDataSet><Table1></Table1></NewDataSet>'){
        ...
        this.lstSalesReturn=this.DBResult.Table;  
      }else{

      }
    });
 
  }else if(this.SalesReturnReport.errors.notValid=='true'  || this.f.ToDate.value==this.FinancialEndYear ){
    this.submitted=true;
  }else{
  ...
  ClearViewData(){
    ...
    this.SalesReturnReport.patchValue({
      CustomerId:0,
