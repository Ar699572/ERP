...
export class SalesInvoiceReportComponent implements OnInit {
  ...
  constructor(private router:Router,private formBuilder: FormBuilder,private datePipe:DatePipe,private APICall:APICallingService,private store: Store<any>) {
    ...
    this.SalesInvoiceReport =this.formBuilder.group({
...
this.LoadFinancialyear()
  ...
  get f(){
    return this.SalesInvoiceReport.controls;
  }
...
FinancialYearxml:string=''
  ...
  GetCustomerInvoices(){
   ...
   if(this.SalesInvoiceReport.errors==null && this.f.FromDate.value <= this.f.ToDate.value && this.f.CustomerId.value !='') {
    debugger;
  ...
  this.APICall.DBCalling("SalesInvoiceReport", xml1, "", "", "").subscribe(
    (res: Response) => {
debugger;
      $("#loaderParent").hide();
      ...
      if(this.DBResult =='<NewDataSet><Table1></Table1></NewDataSet>'){
        ...
        this.lstSalesInvoice=this.DBResult.Table;  
      }else{

      }
    });
 
  }else if(this.SalesInvoiceReport.errors.notValid=='true'  || this.f.ToDate.value==this.FinancialEndYear ){
    this.submitted=true;
  }else{
  ...
  ClearViewData(){
    ...
    this.SalesInvoiceReport.patchValue({
      CustomerId:0,
