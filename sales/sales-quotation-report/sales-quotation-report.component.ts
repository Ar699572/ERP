...
@Component({
  selector: 'app-sales-quotation-report',
  templateUrl: './sales-quotation-report.component.html',
  styleUrls: ['./sales-quotation-report.component.css'],
  providers:[DatePipe]
})
export class SalesQuotationReportComponent implements OnInit {

  FinancialstartYear : string;
  SalesQuotationReport:FormGroup;
  ...
  constructor(private router:Router,private formBuilder: FormBuilder,private datePipe:DatePipe,private APICall:APICallingService,private store: Store<any>) {
    ...
    this.SalesQuotationReport =this.formBuilder.group({
...
this.LoadFinancialyear()
  ...
  get f(){
    return this.SalesQuotationReport.controls;
  }
...
FinancialYearxml:string=''
  ...
  if(this.FinancialYearxml==''){
   ...
   if(this.SalesQuotationReport.errors==null && this.f.FromDate.value <= this.f.ToDate.value && this.f.CustomerId.value !='') {
    debugger;
  ...
  this.APICall.DBCalling("SalesQuotationReport", xml1, "", "", "").subscribe(
    (res: Response) => {
debugger;
      $("#loaderParent").hide();
      ...
      if(this.DBResult =='<NewDataSet><Table1></Table1></NewDataSet>'){
        ...
        this.lstSalesQuotation=this.DBResult.Table;  
      }else{

      }
    });
 
  }else if(this.SalesQuotationReport.errors.notValid=='true'  || this.f.ToDate.value==this.FinancialEndYear ){
    this.submitted=true;
  }else{
  ...
  ClearViewData(){
    ...
    this.SalesQuotationReport.patchValue({
      CustomerId:0,
